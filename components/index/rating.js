    ...
    return(
        <>   
            <section className="rating-container">
                <Image className="img-star p-3 rounded-circle" src="/images/icon-star.svg" width={50} height={50}  alt="Star icon"/>
                <h1 className="my-4">How did we do?</h1>
                <p>Please let us know how we did with your support request. All feedback is appreciated to help us improve our offering!</p>
                <div className="ratings-container d-flex justify-content-between my-4">
                    {ratings.map(rating => (
                        <button key={rating} className="rating rounded-circle border-0" onClick={() => handleRatingSelect(rating)}>{rating}</button>
                    ))}
                </div>
                <button className="submit-btn w-100 border-0 text-uppercase rounded-5 py-2 mt-3" onClick={handleSubmit}>Submit</button>
            </section>
        </>
    );
}